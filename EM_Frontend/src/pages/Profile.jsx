      ...
      <Layout>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-400 mt-2">
            The employee you are trying to view does not exist in the database.
          </p>
          <Button
          ...
          <div className="flex items-center gap-5">
            <Avatar className="h-20 w-20 border-2 border-indigo-100 shadow-sm shrink-0">
              {employee.id === "elena-rodriguez" ? (
                ...
                <AvatarFallback className="bg-indigo-150 text-indigo-700 text-2xl font-black">
                  {initials}
                </AvatarFallback>
              )}
            </Avatar>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-6 lg:col-span-1">
                ...
                <div className="flex gap-3 items-center group">
                  ...
                  <div className="min-w-0">
                    ...
                    <a
                      ...
                      className="text-sm font-semibold text-slate-800 hover:text-indigo-600 transition-colors block"
                    >
                      {employee.phone}
                    </a>
                  </div>
            ...
            <Card className="border-slate-100 shadow-sm bg-white">
              ...
              <CardContent className="pt-4 flex flex-col gap-4">
                {/* Manager */}
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-2.5">
                    Manager
                  </span>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {employee.performanceNote && (
              <Card className="border-slate-100 shadow-sm bg-linear-to-tr from-indigo-50/20 via-white to-violet-50/30">
                  ...
                  <div className="flex items-center justify-between mt-3 text-xs">
                    <span className="text-slate-400 font-medium">
                      ...
                      <span className="font-bold text-slate-600">
                        {employee.performanceNote.author}
                      </span>{" "}
                      ({employee.performanceNote.role})
                    </span>
                    <span className="text-slate-400 font-bold">
                      {employee.performanceNote.date}
                    </span>
                  </div>
                </CardContent>
              </Card>
                          ...
                          {work.tags && work.tags.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                              {work.tags.map((tag) => (
                                <span
                                  key={tag}
                                  className="text-[10px] font-bold bg-slate-50 text-slate-500 border border-slate-150 px-2 py-0.5 rounded-md uppercase tracking-wider"
                                >
                                  {tag}
                                </span>
                              ))}
                    ...
                    <div className="relative group flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs text-slate-400 font-bold">
                          Started {formatJoined(employee.startDate)}
                        </span>
                        <p className="text-xs text-slate-500 mt-1.5">
                          No work history log added.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {employee.skills && employee.skills.length > 0 ? (
                      employee.skills.map((skill) => (
                        <span
                          key={skill}
                          className="text-xs font-semibold bg-indigo-50/50 text-indigo-700 border border-indigo-100/50 px-3 py-1 rounded-lg"
                        >
                          {skill}
                        </span>
                      ))
                ...
                <CardContent className="pt-4 flex flex-col gap-2.5">
                  ...
                  employee.certifications.length > 0 ? (
                    employee.certifications.map((cert) => (
                      <div
                        key={cert}
                        className="flex gap-2 items-center text-xs font-bold text-slate-700"
                      >
              ...
              <Card className="border-slate-100 shadow-sm bg-white">
                ...
                <CardContent className="pt-4 flex flex-col gap-3">
                  {employee.files.map((file) => (
                    <div
                      key={file.name}
                      className="flex items-center justify-between p-3 border border-slate-100 rounded-lg hover:bg-slate-50/50 transition-colors"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-[10px] text-slate-400 font-semibold block">
                            {file.size} &middot; Uploaded {file.date}
                          </span>
                        </div>
                      </div>
                      <a
